#!/usr/bin/env python3
"""Export modified sources, exact dependency inputs and static relinking material."""
import hashlib
import json
import os
...
with tarfile.open(out/('source-materials'+suffix+'.tar.gz'),'w:gz',compresslevel=1, dereference=True) as tar:
    ...
    for name in tracked:
        p=root/name
        if not name or name=='shared/deskport-core' or not p.is_file(): continue
        tar.add(p,arcname='deskport/'+name,recursive=False)
    for name in ('shared/deskport-core','app/deskport.ico','winbuild/scripts','winbuild/toolchain-notices','winbuild/THIRD-PARTY-NOTICES.txt'):
...
with tarfile.open(out/('relink-materials'+suffix+'.tar.gz'),'w:gz',compresslevel=1) as tar:
    for name in ('build-app','prefix','qt-static','compat-include','toolshim'):
        tar.add(wb/name,arcname='deskport/winbuild/'+name)
