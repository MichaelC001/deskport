...
Section "DeskPort" SecMain
  ...
  ${If} $0 != 0
    MessageBox MB_ICONSTOP "DeskPort could not close this installation safely. Close its windows and tray process, then retry."
    Abort
  ${EndIf}
  SetOutPath "$INSTDIR"
  File /r "${PAYLOAD}\*.*"
  ...
  ${If} $0 != 0
  ${AndIf} $0 != 3010
    MessageBox MB_ICONSTOP "Windows rejected or canceled staging the signed display driver (error $0). The installed uninstaller can remove the incomplete installation."
    Abort
  ${EndIf}
  ...
  ${If} $0 == 3010
    SetRebootFlag true
  ${ElseIf} $0 != 0
    MessageBox MB_ICONSTOP "Windows could not install the signed DeskPort virtual display (error $0). No signing policy was changed. The installation is incomplete; the installed uninstaller can remove its files."
    Abort
  ${EndIf}
...
Section "Uninstall"
  ...
  ${If} $0 != 0
    MessageBox MB_ICONSTOP "Close this DeskPort installation before uninstalling."
    Abort
  ${EndIf}
  ExecWait '$\"$INSTDIR\deskport-maintenance.exe$\" remove-recovery $\"$INSTDIR$\"' $0
  ${If} $0 != 0
    MessageBox MB_ICONSTOP "DeskPort could not safely remove its recovery task (error $0)."
    Abort
  ${EndIf}
  ExecWait '$\"$INSTDIR\deskport-driver-setup.exe$\" remove' $0
  ${If} $0 != 0
  ${AndIf} $0 != 3010
    MessageBox MB_ICONSTOP "The DeskPort virtual display could not be removed. Its files and ownership record have been retained."
    Abort
  ${EndIf}
  nsExec::ExecToLog 'netsh advfirewall firewall delete rule name="DeskPort Full Host TCP" program="$INSTDIR\host\deskport-host.exe"'
  Pop $0
  ...
  ${If} $0 != 0
    MessageBox MB_ICONSTOP "The virtual display configuration changed outside DeskPort. Its files were retained (error $0)."
    Abort
  ${EndIf}
  ExecWait '$\"$INSTDIR\deskport-driver-setup.exe$\" remove-package' $0
  ${If} $0 != 0
    MessageBox MB_ICONSTOP "Windows retained the DeskPort driver package (error $0). The ownership record and uninstaller remain available for retry."
    Abort
  ${EndIf}
